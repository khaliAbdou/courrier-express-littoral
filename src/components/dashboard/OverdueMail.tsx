...
interface OverdueMailProps {
  overdueEmails: (IncomingMail | OutgoingMail)[];
}
    ...
    <Card className="mb-8 animate-fade-in">
      <CardHeader>
        <CardTitle className="text-agency-blue flex items-center">
          ...
          Courriers en Retard ({overdueEmails?.length ?? 0})
        </CardTitle>
      </CardHeader>
      <CardContent>
        {!overdueEmails || overdueEmails.length === 0 ? (
          ...
          <div className="space-y-3">
            {overdueEmails.slice(0, 5).map((mail, index) => {
              // Check if it's an incoming mail to access senderName
              const isIncomingMail = 'senderName' in mail;
              const displayName = isIncomingMail 
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{displayName || 'Nom non défini'}</span>
                      </div>
                      <div className="flex items-center gap-1">
            ...
            {overdueEmails.length > 5 && (
              <div className="text-center mt-4 p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Consultez la liste complète des courriers
                </p>
              </div>
            )}
          </div>
